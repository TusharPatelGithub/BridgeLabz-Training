...
namespace FundooApp.Api.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        // Protected endpoint - proves the JWT authorization pipeline actually works.
        // Call this WITHOUT a token first (expect 401), then WITH a valid Bearer token (expect 200).
        [Authorize]
        [HttpGet("profile")]
        public IActionResult GetProfile()
        {
            var claims = User.Claims.Select(c => new { c.Type, c.Value });
            return Ok(new { message = "You are authorized.", claims });
        }
    }
}
