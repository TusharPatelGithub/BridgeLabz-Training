...
namespace BridgeLabz.AddressBookSystem
{
    class AddressBookUtilityImpl : IAddressBook
    {
        ...
        public async Task WriteAddressBookToCSVAsync()
        {
            if (currentAddressBook == null)
                throw new EmptyInputException("Select Address Book first");
            using var writer = new StreamWriter(currentAddressBook + ".csv");
            using var csv = new CsvHelper.CsvWriter(writer, CultureInfo.InvariantCulture);
            await csv.WriteRecordsAsync(addressBooks[currentAddressBook]);
        ...
        public async Task WriteAddressBookToJsonAsync()
        {
            if (currentAddressBook == null)
                throw new EmptyInputException("Select Address Book first");
            string json = JsonSerializer.Serialize(addressBooks[currentAddressBook],
            new JsonSerializerOptions { WriteIndented = true });
            await File.WriteAllTextAsync(currentAddressBook + ".json", json);
